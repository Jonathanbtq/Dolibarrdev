...
/**
 * \file    fastmodulecheck/admin/setup.php
 * \ingroup fastmodulecheck
 * \brief   Fastmodulecheck setup page.
 */
...
require_once '../lib/fastmodulecheck.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "fastmodulecheck@fastmodulecheck"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
$hookmanager->initHooks(array('fastmodulechecksetup', 'globalsetup'));
...
$item = $formSetup->newItem('FASTMODULECHECK_MYPARAM1');
$item->fieldOverride = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
$item->cssClass = 'minwidth500';

// Setup conf for selection of a simple string input
$item = $formSetup->newItem('FASTMODULECHECK_MYPARAM2');
$item->defaultFieldValue = 'default value';
$item->fieldAttr['placeholder'] = 'A placeholder here';

// Setup conf for selection of a simple textarea input but we replace the text of field title
$item = $formSetup->newItem('FASTMODULECHECK_MYPARAM3');
$item->nameText = $item->getNameText().' more html text ';

// Setup conf for a selection of a thirdparty
$item = $formSetup->newItem('FASTMODULECHECK_MYPARAM4');
$item->setAsThirdpartyType();

// Setup conf for a selection of a boolean
$formSetup->newItem('FASTMODULECHECK_MYPARAM5')->setAsYesNo();

// Setup conf for a selection of an email template of type thirdparty
$formSetup->newItem('FASTMODULECHECK_MYPARAM6')->setAsEmailTemplate('thirdparty');
...
$formSetup->newItem('FASTMODULECHECK_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('FASTMODULECHECK_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('FASTMODULECHECK_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('FASTMODULECHECK_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf FASTMODULECHECK_MYPARAM10
$item = $formSetup->newItem('FASTMODULECHECK_MYPARAM10');
$item->setAsColor();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/fastmodulecheck/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=fastmodulecheck-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'FASTMODULECHECK_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'FASTMODULECHECK_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "FastmodulecheckSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-fastmodulecheck page-admin');
...
$head = fastmodulecheckAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "fastmodulecheck@fastmodulecheck");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("FastmodulecheckSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'FASTMODULECHECK_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'FASTMODULECHECK_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
